                                  ...
                                  uint64_t *id)
{
    ...
    for (size_t fd = 0; fd < 3; ++fd)
    {
        if (proc_desc->redirs[fd].type == REDIRECT_FD_PIPE_BLOCKING || proc_desc->redirs[fd].type == REDIRECT_FD_PIPE_CYCLIC)
        {
...
int main(int argc, char **argv)
{
    ...
    while (storage != NULL)
    {
        ...
        CHECK(mount(storage->dev, "/mnt/overlay", storage->fstype, storage->flags, storage->data));
        break;
    }
