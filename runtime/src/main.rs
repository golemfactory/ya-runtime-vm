...
fn convert_result<T>(
    ...
    match result {
        ...
        Err(io_error) => Err(server::ErrorResponse::msg(format!(
            "{} failed, io error: {}",
            msg, io_error
        ))),
    }
...
impl Runtime {
    async fn started<E: server::RuntimeEvent + Send + 'static>(
        ...
        let ga = GuestAgent::connected("./manager.sock", 10, move |notification| {
            event_emiter.on_process_status(notification_into_status(notification));
        })
        .await?;
...
impl server::RuntimeService for Runtime {
    ...
    fn shutdown(&self) -> server::AsyncResponse<'_, ()> {
        ...
        async move {
            ...
            let qemu = data.qemu.take().expect("already shut down");
...
async fn runtime_factory<E: server::RuntimeEvent + Send + 'static>(event_emiter: E) -> Runtime {
    Runtime::started(event_emiter)
        .await
...
async fn main() -> anyhow::Result<()> {
    ...
    match cmdargs.command {
        ...
        Commands::Start { .. } => server::run(runtime_factory).await,
    }
    Ok(())
}
